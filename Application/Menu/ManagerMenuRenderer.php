...
class ManagerMenuRenderer extends AbstractMenuRenderer
{
   ...
   * @return DomElement
   */
  protected function htmlify($menu, $id = '', $class = '', $level = 0)
  {
    ...
    {
      $li = $this->addLiElement($item, $level);
      $id = $item['idMenuItem'];
      if( isset($item['pages']) && $item['pages'] ){
          $li->appendChild( $this->htmlify($item['pages'], 'childs_'.$id, '', $level + 1) );
      }
   ...
   * @return DomElement
   */
  protected function addLiElement($item, $level = 0)
  {
    $id = $item ['idMenuItem'];
    $li = $this->document->createElement('li', str_repeat('&nbsp;&nbsp;', $level*3));
    ...
    $txt = $this->document->createTextNode(utf8_encode($this->getI18n()->_($item['label'])) . $resource  );
