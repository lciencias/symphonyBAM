...
//         	->whereAdd(Notification::ID_BASE_TICKET, $tickets, NotificationQuery::IN)
    ...
    private function process(Notification $notification)
    {
    	...
    	if ($ticket instanceof BaseTicket){
    		...
    		$array = $ticketInfo->toArray();
    		$body = str_replace("%ticket.folio%",$array['%ticket.folio%'],$body);
    		$body = str_replace("%ticket.id_ticket_client%",$array['%ticket.id_ticket_client%'],$body);
    		$body = str_replace("%info.expirationDate%",$array['%ticket.expiration_date%'],$body);
    ...
    private function getVariables(BaseTicket $ticket, TemplateEmail $templateEmail, TicketInfo $ticketInfo)
    {
        $info = array();
        try{
        $info['expirationDate'] = $this->serviceLevelService->getExpirationDateNvo($ticket);
        ...
        $info['percentageService'] = $datosAvance[1]. '%';
        }catch(Exception $e){
        	$info['expirationDate'] = '-';
        	$info['expiredTime'] = '-';
