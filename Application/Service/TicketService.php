...
//            $status = array_search(self::INITIAL_STATUS, Ticket::$Statuses);
            ...
            if($params['controller'] == 'ticket'){
            	...
            	$ticket->setAccountNumber($params['account_number']);
            	$ticket->setNameClient($params['name_client']);
    ...
    public function updateTicket(TicketClient $ticket){
    	try{
    		...
    		$this->getTicketClientCatalog()->beginTransaction();
    		$cambios  = 0;
    		if( ($ticket->getIdTicketType() == TicketType::$TicketType['Aclaraci�n']) || ($ticket->getIdTicketType() == TicketType::$TicketType['Queja']) ){
    ...
    public function getUpdatedateLogs($ticketClient,$userOrigin){
    	$now = new \Zend_Date();    	
    	...
    	/*if(count($ticketLogs) > 0){
   		...
   		$ticketLog2 = TicketLogFactory::createFromArray(array(
   				'id_base_ticket' => $ticketClient->getIdBaseTicket(),
   				'id_user' => $userOrigin,
   				'date_log' => $now->get('yyyy-MM-dd HH:mm:ss'),
