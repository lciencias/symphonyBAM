...
//      @$content=@file_get_contents($url, false, $context);
	 ...
	 * 
	 */
	public function escribeLog($response , $url){
		$filename = "cache/log/LogBam_".date("Y-m-d").".log";
		$buffer   = serialize($response);
 		if(count($response) > 0){
 			$f = fopen($filename, "a+");
 			fwrite($f, "Log dia y hora: ".date("Y-m-d H:i:s")."\n".$url."\n");
 			fwrite($f, $buffer);
 			fwrite($f, "\n*************************************\n");
 			fclose($f);
 		}
...
//                        print_r($log['verLog']['conceptos']['concepto']);
		...
		if(!$this->getDevelopmentMode() || $this->getDevelopmentMode()==''){
                        ...
                        if($datos['error'])
                            ...
                            //$datos['url'] = "http://10.14.2.241/Symphony/public/images/logos/logo.png";
                            //$datos['url'] = "http://www.csasports.com.mx";
                            $datos['exist']= 0;
                            if(trim($datos['url']) != ''){
                            	$datos['exist']= 1;
                            }
                    ...
                    if($type ==7){
                     ...
                     if($type ==8){
                        ...
                        if($datos['error']) return $datos;
                        	...
                        	print_r($datos['detalleTransaccionTipo']);
                        	die();
                        	return $datos['detalleTransaccionTipo'];
                        }
                    }
...
//          $datos=json_decode(file_get_contents($uri),true);
            ...
            foreach($datos['cuentasCliente']['cuentas'] as $data1){
            	foreach($data1 as $key =>$data){
                    ...
                    $fields['type']    = utf8_decode($data1[$key]['tipoCuenta']);
