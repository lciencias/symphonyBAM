    ...
    public function indexAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            $sql=" SELECT tc.id_ticket_client,tc.folio,
            		...
            		tct.good_faith_amount as good_folio,
            ...
            if(count($data)>0)
            {
                ...
                $headers=array('Reporte','N�mero secuencia','Folio','Fecha de Aclaraci�n','Fecha Suceso','N�mero de Cuenta','Producto Catalogo CNBV',
                		'Canal Catalogo CNBV','Motivo Catalogo CNBV',
                		'Importe Reclamado','Estado','Resoluci�n','Fecha Resoluci�n','Causa Resoluci�n','Importe Abonado',
                		'Fecha Abonado','Importe Recuperado','Quebranto','Origen');
                $headers= $fixManager->utf8_encode_array($headers);

                $excel->getActiveSheet()->setCellValueByColumnAndRow(0, 1 ,utf8_encode("Nombre de la instituci�n: "));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, 1 ,utf8_encode("Banco Autofin M�xico S.A Instituci�n de Banca Multiple"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 1 ,utf8_encode("Sector: "));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(4, 1 ,utf8_encode("Instituciones de Banca M�ltiple"));
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 2 ,utf8_encode("N�mero de Aclaraciones"));
						...
						if($row['is_recovered_amount'] == 1){
							...
							if(trim($row['partities'])!= ''){
		                    	...
		                    	//lienas insertadas
		                    		...
		                    		$excel->getActiveSheet()->setCellValueByColumnAndRow(16, $i ,number_format($row['good_folio'],2,'.',','));
		                    	}
		                    }
		                    else{
		                    	if(trim($row['good_folio']) == ''){
		                    ...
		                    if(trim($row['partities'])!= ''){
		                    	...
		                    	if((trim(utf8_encode($row['tipo'])) == '1') ){
		                    		if(trim($row['good_folio']) == ''){
		                    			...
		                    			$excel->getActiveSheet()->setCellValueByColumnAndRow(17, $i ,number_format($row['good_folio'],2,'.',','));
		                    		}
		                    	}
		                    	else{
		                    		$excel->getActiveSheet()->setCellValueByColumnAndRow(17, $i ,"");
    ...
    public function reuneAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            if(count($data)>0)
            {
                ...
                $headers=array('Folio','Estado:','Fecha de notificaci�n al usuario','Entidad federativa',
                        'Fecha de recepci�n','Medios de recepci�n','Producto y servicio','Causa');
                $headers= $fixManager->utf8_encode_array($headers);
                $excel->getActiveSheet()->setCellValueByColumnAndRow(0, 1 ,utf8_encode("Nombre de la instituci�n: "));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, 1 ,utf8_encode("Banco Autofin M�xico S.A Instituci�n de Banca Multiple"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 1 ,utf8_encode("Sector: "));
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(0, 2 ,utf8_encode("Trimestre a Informar"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, 2 ,$trimester);
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 2 ,utf8_encode("N�mero de Consultas"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(4, 2 ,count($data));
                $excel->getActiveSheet()->getStyle('A1:E1')->getFont()->setBold(true);
                $excel->getActiveSheet()->getStyle('A2:E2')->getFont()->setBold(true);
    ...
    public function reuneAclAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            if(count($data)>0)
            {
                ...
                $excel = new ReportExcel();
                $excel->setActiveSheetIndex(0);
                $headers=array('Folio','Estado de concluido o pendiente','Fecha de resoluci�n',
                        'Fecha de notificaci�n al usuario','Entidad federativa',
                        'Fecha de recepci�n','Medios de recepci�n','Producto y servicio','Causa',"Importe reclamado","Importe que se restituy� al Usuario",
                    	"Fecha en que se restituy� el importe al Usuario","Folio Inst","Reversa SIGEO");
                $headers= $fixManager->utf8_encode_array($headers);
                $excel->getActiveSheet()->setCellValueByColumnAndRow(0, 1 ,utf8_encode("Nombre de la instituci�n: "));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, 1 ,utf8_encode("Banco Autofin M�xico S.A Instituci�n de Banca Multiple"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 1 ,utf8_encode("Sector: "));
    ...
    public function reuneRecAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            if(count($data)>0)
            {
                ...
                $excel = new ReportExcel();
                $excel->setActiveSheetIndex(0);
                $headers=array('Folio','Estado','Fecha de resoluci�n','Fecha de notificaci�n al usuario','Entidad federativa',
                        'Fecha de recepci�n','Medios de recepci�n','Producto y servicio','Causa','Importe reclamado',
                		'Importe que se restituy� al Usuario','Fecha en que se restituy� el importe al Usuario','Folio Inst','Reversa SIGEO');
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(0, 2 ,utf8_encode("Trimestre a Informar"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, 2 ,$trimester);
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, 2 ,utf8_encode("N�mero de Reclamaciones"));
                $excel->getActiveSheet()->setCellValueByColumnAndRow(4, 2 ,count($data));
                $excel->getActiveSheet()->getStyle('A1:E1')->getFont()->setBold(true);
                $excel->getActiveSheet()->getStyle('A2:E2')->getFont()->setBold(true);
                $excel->getActiveSheet()->getStyle('A3:N3')->getFont()->setBold(true);
