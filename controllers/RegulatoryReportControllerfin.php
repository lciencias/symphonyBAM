...
class RegulatoryReportController extends BaseController{
    ...
    public function indexAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            $sql=" SELECT tc.id_ticket_client,tc.folio,
            		convert(varchar,bt.created, 103) as reclamation_date,
					...
					CASE WHEN bt.status IN(5,7,8) THEN '402' else '401' END AS estado,res.name as resolution,
					...
					WHERE tc.expiration_date is not null AND bt.id_ticket_type=4  AND bt.status IN (3,5) 
           	      		AND 
            			(
            			  (bt.created >='".$params['ini']."'  and bt.created < '$fin' )
            				OR             			  
            			  (ass.resolution_date >='".$params['ini']."'  and ass.resolution_date < '$fin')
            			 )
            		ORDER BY tc.id_ticket_client,bt.created,ass.resolution_date,tct.transaction_date";
                	...
                	if(substr(trim($row['folio']),0,1) != 'P'){                	
						...
						if($row['is_recovered_amount'] == 1){                    	
		                    ...
		                    if(trim($row['partities'])!= ''){
		                    	...
		                    	if((trim(utf8_encode($row['typ'])) != '502') && (trim(utf8_encode($row['typ'])) != '503') ){
    ...
    public function reuneAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            $sql=" SELECT tc.folio, CASE WHEN bt.status IN(1,2,3,5,6,7,8) THEN 'CONCLUIDO' else 'PENDIENTE' END AS estado_ticket,
            		...
            		tc.id_entidad AS entidad,convert(varchar,bt.created, 103) as fecha_recepcion,c.canal_acl as medio,
			...
			LEFT JOIN pcs_symphony_assignments ass on ass.id_base_ticket=bt.id_base_ticket
			LEFT JOIN pcs_symphony_client_resolutions res on res.id_client_resolution=ass.id_resolution
			WHERE bt.id_ticket_type=3  and bt.created >='".$params['ini']."'  and bt.created < '$fin' AND bt.status IN (3,5) ";
            $data=$this->getReasonsCatalog()->getDb()->fetchAll($sql);
            if(count($data)>0)
            {
                $this->view->setTpl("Index")->setLayoutFile(false);            
                ...
                foreach($data as $row){
                	if(trim($row['folio']) != ''){
	                    ...
	                    if($row['id_resolution'] != null && $row['id_resolution'] != ''){
	                    	$row['estado_ticket'] = 402;
	                    }
	                    else{
	                    	$row['estado_ticket'] = 401;
	                    }
	                    ...
	                    $excel->getActiveSheet()->setCellValueByColumnAndRow(2, $i ,utf8_encode($row['fecha_notificacion']));
    ...
    public function reuneAclAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            $nuevafecha = strtotime ( '+ 3 month' , strtotime ( $params['ini'] ) ) ;
            $fin = date ( 'Y-m-d' , $nuevafecha );
            $sql="SELECT tc.id_ticket_client,tc.folio,tc.folio_prev,CASE WHEN bt.status IN(5,7,8) THEN 'CONCLUIDO' else 'PENDIENTE' END AS estado_ticket, 
					convert(varchar,ASS.resolution_date, 103) as fecha_resolucion, convert(varchar,ASS.resolution_date, 103) as fecha_notificacion, tc.id_entidad AS entidad,convert(varchar,bt.created, 103) as fecha_recepcion,
					c.canal_acl as medio, convert(varchar,bt.created, 103) as reclamation_date,tct.transaction_date as event_date,tc.account_number,p.name as producto,
			 		r.name as causa,tct.amount as claimed, CASE WHEN bt.status IN(5,7,8) THEN 'CONCLUIDO' else 'PENDIENTE' END AS estado,
			  		...
			  		tct.amount as importe,
			 		CASE
			 			WHEN (tct.good_faith_amount ='' OR tct.good_faith_amount IS NULL ) THEN tct.amount
			 			ELSE tct.good_faith_amount
			 		END AS importe_usuario,
			 		CASE
			 			WHEN (tct.good_faith_amount ='' OR tct.good_faith_amount IS NULL ) THEN convert(varchar,ass.resolution_date, 103)
			 			ELSE convert(varchar,tct.good_faith_date, 103)
			 		END AS  fecha_importe,
			 		'' as folio_ins,
			 		'0' as reversa,bt.status,res.type,ass.id_resolution, tct.amount as monto,convert(varchar,ass.resolution_date, 103) as resolutionDate,
            		convert(varchar,tct.good_faith_date, 103) as good_faith_date,good_faith_amount,
			  		tct.good_faith_date as ammount_date, case when ass.is_recovered_amount='1' then tct.amount else '' end as recovery_amount,
              ...
              WHERE bt.id_ticket_type=4 AND  
            		(bt.created >='".$params['ini']."'  and bt.created < '$fin' or ass.resolution_date >='".$params['ini']."'  and ass.resolution_date < '$fin')
            		AND bt.status IN (3,5)";
            $data=$this->getReasonsCatalog()->getDb()->fetchAll($sql);
            if(count($data)>0)
            {
                $this->view->setTpl("Index")->setLayoutFile(false);            
                ...
                foreach($data as $row){
                	$semilla = trim($row['id_ticket_client']).'-'.trim($row['id_transaction']);
                	//if(trim($row['folio']) != '' && substr(strtoupper(trim($row['folio'])),0,1) != 'P'){
	                	//if(!in_array($semilla,$array_unicos)){
	                		$array_unicos[] = $semilla;
		                    ...
		                    if($row['id_resolution'] != null && $row['id_resolution'] != ""){
		                    	if((int) $row['type'] == 1){
		                    		$excel->getActiveSheet()->setCellValueByColumnAndRow(10, $i ,utf8_encode(str_replace('-','',$row['good_faith_amount'])));
		                    		$excel->getActiveSheet()->setCellValueByColumnAndRow(11, $i ,utf8_encode($row['good_faith_date']));
		                    	}else{
		                    		//$excel->getActiveSheet()->setCellValueByColumnAndRow(10, $i ,utf8_encode(str_replace('-','',$row['monto'])));
		                    		//$excel->getActiveSheet()->setCellValueByColumnAndRow(11, $i ,utf8_encode($row['resolutionDate']));
    ...
    public function reuneRecAction()
    {
        ...
        if( $this->getRequest()->isPost() )
        {
            ...
            $nuevafecha = strtotime ( '+ 3 month' , strtotime ( $params['ini'] ) ) ;
            $fin = date ( 'Y-m-d' , $nuevafecha );
            $sql="SELECT tc.id_ticket_client,tc.folio,CASE WHEN bt.status IN(5,7,8) THEN 'CONCLUIDO' else 'PENDIENTE' END AS estado_ticket,
                ...
                convert(varchar,bt.created, 103) as reclamation_date,convert(varchar,tct.transaction_date, 103) as event_date,tc.account_number,p.name as producto,
				r.name as causa,tct.amount as claimed, 
				CASE WHEN bt.status IN(5,7,8) THEN 'CERRADO' else 'PENDIENTE' END AS estado,
				res.name as resolution,bt.scheduled_date as resolution_date,ass.note as resolution_cause,
				tct.good_faith_amount as importe,tct.good_faith_date as ammount_date,
				case when ass.is_recovered_amount='1' then tct.amount else '' end as recovery_amount,
				case when ass.is_recovered_amount='0' then tct.amount else '' end as  quebranto, 
				c.canal_recl as origen,r.prod_ser_Rec,r.causa_rec,
				tctn.num,tct.id_ticket_client_transaction as id_transaction,tc.folio_condusef,
	            reop.amount as reopen_amount,bt.status,res.type,ass.id_resolution,
            ...
            if(count($data)>0)
            {
                ...
                $excel = new ReportExcel();
                $excel->setActiveSheetIndex(0);
                $headers=array('Folio','Estado','Fecha de resolución',
                        'Fecha de notificación al usuario','Entidad federativa',
                        'Fecha de recepción','Medios de recepción','Producto y servicio','Causa',"Importe reclamado","Importe que se restituyó al Usuario",
                    "Fecha en que se restituyó el importe al Usuario","Folio Inst","Reversa SIGEO");
                $headers= $fixManager->utf8_encode_array($headers);
                ...
                foreach($data as $row){
					...
					if(trim($row['folio_condusef']) == ''){                        
	                	...
	                	$excel->getActiveSheet()->setCellValueExplicit("A$i",utf8_encode($row['folio_condusef']),PHPExcel_Cell_DataType::TYPE_STRING);
	                }
	                if(in_array($semilla,$array_unicos)){
	                	$row['estado_ticket'] = 403;
	                    $row['fecha_notificacion'] = $row['fecha_resolucion'];
	                    $row['fecha_resolucion']= "";
	                }else{
	                	if($row['id_resolution'] != null && $row['id_resolution'] != ''){
