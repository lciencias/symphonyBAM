...
// 			return false;
	...
	public function createAction(){
		if($this->getRequest()->isPost()){
			...
			if( self::validateParameter($params) == 1) {
				...
				$this->setFlash('error', "Error: informaci�n incompleta");
				$this->_redirect('ticket-client/new');
			}
		}
	 ...
	 function validateParameter($params){
		...
		if (empty($params['no_card'])){
			return false;
		}
		if (empty($params['name_client'])){
			return false;
		}
		if ((int) ($params['id_product']) == 0){
			return false;
		}
		if (empty($params['client_number'])){
			return false;
		}
		if (empty($params['account_type'])){
			return false;
		}
		if (empty($params['id_client_category'])){
			return false;
		}
		...
		if (empty($params['id_origin_branch'])){
			return false;
		}
		if (empty($params['id_reported_branch'])){
			return false;
		}
	...
	public function resolveAction(){
		try {
			...
			if(trim($_FILES['file']['name']) != ""){
				...
				if($file != null){  //Linea agregada 241017
					$this->getFileCatalog()->create($file);
				}
			}			
...
// 			echo $e->getMessage();
	...
	public function reportPdfAction(){
		...
		if(count($information) == 0 && (int)$params['idClientTicket'] > 0){
			$ticketClient = TicketClientQuery::create()->findByPK($params['idClientTicket']);
			$clientCategory    = ClientCategoryQuery::create()->findByPK($ticketClient->getIdClientCategory());
			$branchesBam = BranchQuery::create()->findByPK($ticketClient->getIdOriginBranch());
			$query_created = "SELECT convert(varchar,created, 103) as date_created FROM pcs_symphony_base_tickets WHERE id_base_ticket=".$ticketClient->getIdBaseTicket();
			$date_created=$this->getTicketClientFieldCatalog()->getDb()->fetchAll($query_created);
			$information  = $ticketClient->toArray();
			$information['folio']  		   = $ticketClient->getFolio();
			$information['folio_prev']  	   = $ticketClient->getFolioPrev();
			$information['id_base_ticket']  = $ticketClient->getIdBaseTicket();
			$information['account_number']  = $ticketClient->getAccountNumber();
			$information['client_number']   = $ticketClient->getClientNumber();
			$information['name']  		   = $ticketClient->getNameClient();
			$information['email']  		   = $ticketClient->getEmail();
			$information['account_type']    = $ticketClient->getAccountType();
			$information['employee']   	   = $ticketClient->getEmployee() == 1 ? "Si":"No";
			$information['telefono']    	   = $ticketClient->getTelefono();
			if($ticketClient->getCardType()==0)$cardType="N/A";
			if($ticketClient->getCardType()==1)$cardType="Si";
			if($ticketClient->getCardType()==2)$cardType="No";
			$information['card_type']   = $cardType;
			$information['no_card']    = $ticketClient->getNoCard();
			$information['id_branch']  = $ticketClient->getIdOriginBranch();
			$information['branch']     = self::getBranch($ticketClient->getIdOriginBranch());
			$information['id_bam']     = $branchesBam->getIdBam();
			$information['client_category_name']   = $clientCategory->getName();
			$information['description_report']     = $ticketClient->getDescription();
			$information['effect_partial']     	   = $effect_partial[0]['total'];
			$information['created'] 			   = $date_created[0]['date_created'];
			$moreInfo= TicketClientQuery::create()->addColumn("Products.name","product")
			->addColumn("Channel.name","channel")
			->innerJoinProducts()
			->innerJoinChannel2()
			->whereAdd(TicketClient::ID_TICKET_CLIENT,$params['idClientTicket'])->fetchAll();
			$information['product']=$moreInfo[0]['product'];
			$information['channel']=$moreInfo[0]['channel'];
		}
		$this->getTicketService()->updateEmail($params);		
		$this->report($params, $information, $transaction,$documents,0);
	}
	
	public function reportPdfAnterior(){
		$params = $this->getRequest()->getParams();
		$params['templateEmail'] = (int) TemplateEmailQuery::create()->whereAdd(TemplateEmail::EVENT, EmailEvent::TICKET_CLIENT_FORMAT_ACL)->fetchOne();		
		$params['ajaxJason'] = 1;
		$information = json_decode($params['clientInformation'],true);
	...
	private function report($params, $information, $transaction,$documents,$idBaseTicket = 0){
		$idBaseTicketTmp = $information['id_base_ticket'];
		if((int) $idBaseTicketTmp == 0){			
            $sql = "SELECT id_base_ticket FROM pcs_symphony_tickets_clients 
            		WHERE id_ticket_client='".$params['id_ticket_client']."' ORDER BY id_ticket_client desc;";
		    $resul=$this->getTicketClientFieldCatalog()->getDb()->fetchAll($sql);
		    $idBaseTicketTmp = $resul[0]['id_base_ticket']; 			
		}
...
//                                $data['created']=($fecha_creado)?$fecha_creado[2]."/".$fecha_creado[1]."/".$fecha_creado[0]:"";
			...
			if( (int) $params['format'] == 2){
				...
				$now = new \Zend_Date();
				$notification= new Notification();
				$fields2=array("id_template_email" => $params['templateEmail'],"to"=> $params['emailClient'],"id_base_ticket"=>$idBaseTicketTmp,"created"=>$now->get('yyyy-MM-dd HH:mm:ss'),"id_file" => $idFile );
				NotificationFactory::populate($notification, $fields2);
				$this->getNotificationCatalog()->create($notification);
