...
final class TicketClientController extends CrudController{
	...
	public function listAction(){
		...
		$this->view->users = UserQuery::create()->actives()->find()->toCombo($header);
		$this->view->statuses = $this->getStatusCombo($header);
		$this->view->tickets = TicketClientQuery::create()->filter($params)
	...
	public function listByGroupAction(){
		...
		$this->view->users = UserQuery::create()->actives()->find()->toCombo($header);
		$this->view->statuses = $this->getStatusCombo($header);
		$this->view->tickets = TicketClientQuery::create()
		->filter($params)
		->whereAdd(TicketClient::ID_CLIENT_CATEGORY, $clientCategory->getPrimaryKeys())
		->orderBy(TicketClient::ID_TICKET_CLIENT,TicketClientQuery::DESC)
		->page($page, $this->getMaxPerPage())
	...
	public function reopenAction(){
		...
		try {
			...
			if($ticket != null){
				...
				$this->getTicketService()->getUpdatedateLogs($ticket);
			}
				...
				if ($group->getIdUserAssignedForTickets ()) {
					...
					try {
						...
						if($ticketClientT != null){
							...
							$this->getTicketService()->getUpdatedateLogs($ticketClientT);
						}
		...
		if($this->getRequest()->isPost()){
			try{
				...
				if($transaction != null){
					...
					/*if(!empty($params['id_controversy_chargeback_d'])){
						$transaction->set($params['id_controversy_chargeback_d']);
					}
					if(!empty($params['id_controversy_chargeback_t'])){
						$transaction->set($params['id_controversy_chargeback_t']);
					}*/
