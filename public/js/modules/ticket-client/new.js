...
/*
		...
		if(parseInt($("#id_ticket_client").val()) > 0){
        	...
        	$.ajax({																
        		...
        		data : {
        			...
        			idClientTicket    : $("#id_ticket_client").val(),
        			format:1,
        			type:2,        			
        		},
        		error : function(){
	...
	$(".transacciones").click(function(){
		...
		$(".errorControversia").html("");
		$(".good_faith_payment_request").val("");
		$(".good_faith_payment_request").css({display:'none'});
		$(".good_faith_delivery_request").val("");
		$(".good_faith_delivery_request").css({display:'none'});
		$(".buttonAbonoBuenaFe").css({display:'none'});
		$(".buttonFolioBuenaFe").css({display:'none'});
		...
		if(parseInt(tmp[0]) >  0 && parseInt(tmp[1]) >=  0 ){
			...
			$('#my-modal-'+tmp[1]).find("#idTransactionPayment").val(tmp[0]);
			$('#my-modal-'+tmp[1]).find("#idPermisoPayment").val(tmp[4]);
			$('#my-modal-'+tmp[1]).find(".abonoBuenaFeBoton").css({display:'block'});
			if(parseInt(tmp[4]) === 2){
				$('#my-modal-'+tmp[1]).find(".abonoBuenaFeBoton").css({display:'none'});
			}
			if(parseInt(tmp[5]) === 2){
				$('#my-modal-'+tmp[1]).find(".rabonoBuenaFeBoton").css({display:'none'});
			}
			
			$('#my-modal-'+tmp[1]).find("#nmTransaction").html("ID T24: "+tmp[3]);
			$.ajax({																
				...
				success : function(Info){
					if(Info == null){
						...
						if(parseInt(tmp[1]) !== 8){
							...
							if(Info.partial[0].amount_partial !== undefined && Info.partial[0].deposit_date_partial !== undefined){
								$("#amountDepositedModal"+tmp[1]).html(Info.partial[0].amount_partial);
								$("#dateDepositedModal"+tmp[1]).html(Info.partial[0].deposit_date_partial);							
								if(Info.partial[0].voucher !== undefined){
						...
						case 0:
							...
							default:
								...
								$('#my-modal-'+tmp[1]).find(".rabonoBuenaFeBoton").css({display:'none'});
								$('#my-modal-'+tmp[1]).find(".good_faith_payment").html("");
								$('#my-modal-'+tmp[1]).find(".good_faith_date").html("");
								$('#my-modal-'+tmp[1]).find(".good_faith_amount").html("");
								$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_d").val('');
								$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_t").val('');
								$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_d").empty()
								$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_t").empty()
								$('#my-modal-'+tmp[1]).find("#id_controversy_reason").val('');
								$('#my-modal-'+tmp[1]).find("#type").val('');
								$('#my-modal-'+tmp[1]).find("#debit_time").val('');
								$('#my-modal-'+tmp[1]).find("#condiciones").html('');
								$('#my-modal-'+tmp[1]).find("#documentos").html('');
								$('#my-modal-'+tmp[1]).find("#representacion").html('');
								$('#my-modal-'+tmp[1]).find("#payment_request_date").val('');
								$('#my-modal-'+tmp[1]).find("#payment_delivery_date").val('');
								$('#my-modal-'+tmp[1]).find("#accepted_payment").val('');								
								$('#my-modal-'+tmp[1]).find("#delivery_payment").val('');

								
								
								//Abono de buena fe
								dataBd = Info.transactionBd[0];
								$('#my-modal-'+tmp[1]).find(".good_faith_payment").html(dataBd.good_faith_payment);
								$('#my-modal-'+tmp[1]).find(".good_faith_date").html(dataBd.good_faith_date);
								$('#my-modal-'+tmp[1]).find(".good_faith_amount").html(dataBd.good_faith_amount);
								
								if(String(dataBd.good_faith_request) != null && String(dataBd.good_faith_request) != undefined ){
									$('#my-modal-'+tmp[1]).find(".rabonoBuenaFeBoton").css({display:'block'});
								}
								
								//controversia
								if(parseInt(dataBd.type) == 1){
									$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_d").val(dataBd.id_controversy_chargeback);
									$('#my-modal-'+tmp[1]).find(".controversy_chargeback_d").html(dataBd.name);
									$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_t").val('');
									$('#my-modal-'+tmp[1]).find(".controversy_chargeback_t").html('');
								}
								if(parseInt(dataBd.type) == 2){
									$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_d").val('');
									$('#my-modal-'+tmp[1]).find(".controversy_chargeback_d").html('');
									$('#my-modal-'+tmp[1]).find("#id_controversy_chargeback_t").val(dataBd.id_controversy_chargeback);
									$('#my-modal-'+tmp[1]).find(".controversy_chargeback_t").html(dataBd.name);
								}
								$('#my-modal-'+tmp[1]).find("#id_controversy_reason").val(dataBd.id_controversy_reason);
								$('#my-modal-'+tmp[1]).find(".controversy_reason").html(dataBd.reason);
								
								$('#my-modal-'+tmp[1]).find("#type").val(dataBd.reason_type);
								$('#my-modal-'+tmp[1]).find(".type").html(dataBd.reason_type);
								
								
								$('#my-modal-'+tmp[1]).find("#debit_time").val(dataBd.debit_time);
								$('#my-modal-'+tmp[1]).find(".debit_time").html(dataBd.debit_time);
								
								//controversia tabs
								$('#my-modal-'+tmp[1]).find("#condiciones").html(dataBd.type1);
								$('#my-modal-'+tmp[1]).find("#documentos").html(dataBd.type2);
								$('#my-modal-'+tmp[1]).find("#representacion").html(dataBd.type3);
																
								//Solicitud de pagare
								$('#my-modal-'+tmp[1]).find("#payment_request_date").val(dataBd.payment_request_date);
								$('#my-modal-'+tmp[1]).find(".payment_request_date").html(dataBd.payment_request_date);
								$('#my-modal-'+tmp[1]).find("#payment_delivery_date").val(dataBd.payment_delivery_date);
								$('#my-modal-'+tmp[1]).find(".payment_delivery_date").html(dataBd.payment_delivery_date);
								if(dataBd.accepted_payment === null)
									dataBd.accepted_payment = '';
								$('#my-modal-'+tmp[1]).find("#accepted_payment").val(dataBd.accepted_payment);		
								
								if(parseInt(dataBd.accepted_payment) == 1)
									$('#my-modal-'+tmp[1]).find(".accepted_payment").html("Si");
								else
									$('#my-modal-'+tmp[1]).find(".accepted_payment").html("No");
								
								
								if(dataBd.delivered_payment === null)
									dataBd.delivered_payment = '';								
								$('#my-modal-'+tmp[1]).find("#delivery_payment").val(dataBd.delivered_payment);
								
								if(parseInt(dataBd.delivered_payment) == 1)
								...
								$('#my-modal-'+tmp[1]).find(".download"+tmp[1]).empty();
								if(parseInt(dataBd.file_payment) > 0){									
									$('#my-modal-'+tmp[1]).find(".download"+tmp[1]).append("<button name='"+dataBd.file_payment+"' id='"+dataBd.file_payment+"' class='btn default download'>Descargar</button>");
									$('#my-modal-'+tmp[1]).find(".download").click(function(){
										if(parseInt($('#my-modal-'+tmp[1]).find(".download").attr('id')) >0){
											descargaFile($('#my-modal-'+tmp[1]).find(".download").attr('id'));
										}
								...
								//Informacion
								if(Info.transaction[0] != null){
									dataTrans = Info.transaction[0];
								
									$('#my-modal-'+tmp[1]).find(".idT24").html(dataTrans.IDT24);
									$('#my-modal-'+tmp[1]).find(".fechaTxn").html(dataTrans.Fechadetxn);
									$('#my-modal-'+tmp[1]).find(".horaTxn").html(dataTrans.Horadetxn);
									$('#my-modal-'+tmp[1]).find(".importe").html(dataTrans.Importe);
									$('#my-modal-'+tmp[1]).find(".respuesta").html(dataTrans.Respuesta);
									$('#my-modal-'+tmp[1]).find(".tipoTxn").html(dataTrans.Tipodetxn);
									$('#my-modal-'+tmp[1]).find(".motivo").html(dataTrans.Motivoderechazo);
									$('#my-modal-'+tmp[1]).find(".comercio").html(dataTrans.Comercio);
									$('#my-modal-'+tmp[1]).find(".auth").html(dataTrans.NoAuth);
									$('#my-modal-'+tmp[1]).find(".giro").html(dataTrans.Giro);
									$('#my-modal-'+tmp[1]).find(".afiliacion").html(dataTrans.Afiliacion);
									$('#my-modal-'+tmp[1]).find(".pem").html(dataTrans.PEM);
									$('#my-modal-'+tmp[1]).find(".secuencia").html(dataTrans.Secuencia);
									$('#my-modal-'+tmp[1]).find(".referencia").html(dataTrans.Referencia);
									$('#my-modal-'+tmp[1]).find(".respuestaArqc").html(dataTrans.RespuestaArqueo);
									...
									nmCombo = "";
									combo1 = combo2 = "";
									contracargos = Info.contracargos[0]; 
			...
			$.ajax({																
				...
				success : function(info){
					if(info === null){
						...
						exito(info.mensaje);
						if(parseInt($("#id_ticket_client").val()) > 0){
							setTimeout(function(){location.href=baseUrl+"/ticket-client/edit/id/"+$("#id_ticket_client").val()},200);
						}
	...
	$("#guardar").click(function(){		
		if(String($("#account_number").val()) === ''){
			if(String($("#accountTmp").val()) !== ''){
				$("#account_number").val($("#accountTmp").val());
			}
		}
		...
		if(String($("#id_origin_branch").val()) === ''){
			error(I18n._("Required field"));			
			return false;
		}		
		if(String($("#id_reported_branch").val()) === ''){
			error(I18n._("Required field"));			
			return false;
		}		
                ...
                if(accountInfo.error){
							...
							if(productIds.length > 0){
								...
								for(var k = 0; k < (productIds.length-1); k++){
									...
									if(String($("#accountTmp").val()) === String(accountInfo[indexj2].account)){
										...
										accountTable.find('#d'+accountInfo[indexj2].account).append('<button id="'+accountInfo[indexj2].account+"-"+propierts[0]+"-"+propierts[1]+"-"+propierts[2]+"-"+accountInfo[indexj2].type.trim()+"-"+accountInfo[indexj2].card_number+'" '+regresaClase(propierts[0])+' name="'+accountInfo[indexj2].account+"-"+propierts[0]+"-"+propierts[1]+"-"+propierts[2]+'" class="btn default">'+propierts[1]+'</button>&nbsp;&nbsp;');
									} 								
								}
							}
						}
					...
					$('#clientInformationContainer').find('.nvoTicket').click(function(){
						...
						if(String(tmpData[0]) !== "" && parseInt(tmpData[1]) > 0){
							cadena = tmpData[0]+"-"+tmpData[1]+"-"+tmpData[2]+"-"+tmpData[3]+"-"+tmpData[4]+"-"+tmpData[5];
		...
		error : function(){
			...
			if(productInfo == null){
             ...
             if(productInfo.error){
                 ...
                 error(I18n._(productInfo.error));
            }
						...
						if(productIds.length > 0){
							...
							for(var k = 0; k < (productIds.length-1); k++){
								if(String(tmpAccount) === String(productInfo[indexj3].account)){
									propierts = productIds[k].split("|");			
									buttons+='<button id="'+tmpAccount+"-"+propierts[0]+"-"+propierts[1]+"-"+propierts[2]+"-"+productInfo[indexj3].type.trim()+"-"+productInfo[indexj3].card_number+'" '+regresaClase(propierts[0])+' name="'+tmpAccount+"-"+propierts[0]+"-"+propierts[1]+"-"+propierts[3]+"-"+productInfo[indexj3].type+'">'+propierts[1]+'</button>&nbsp;&nbsp;';													
								}
							}
						}
					}
...
function seleccionaProducto(clientFullInfo,account,productId,arrayQuestion){
	...
	if(String(productId) != ""){
		...
		if(String(datos[4]) !== undefined){
			$("#account_type").val(datos[4]);	
		}else{
			$("#account_type").val("N/A");
		}
...
//			$('#newForm').find('#id_origin_branch_tmp').attr('disabled',true);
		...
		if(String(clientFullInfo.id_branch) != ''){
			if(parseInt(clientFullInfo.id_branch) > 0){
				...
				$('#newForm').find('#id_origin_branch').val(264);
				$('#newForm').find('#id_origin_branch_tmp').val(264);
				$('#newForm').find('#id_origin_branch_tmp').attr('disabled',true);				
			}
		}
		else{
		...
		$('#newForm').find('#id_reason').change(function(event){
			event.preventDefault();
			var tmpReason;  
