...
<form method="POST" action="">
    ...
    <table class="zebra-striped bordered-table">
        <caption><h3>{$i18n->_('Employee')}<div style="float: right;">{if "employee/new"|isAllowed}<a href="{url action=new}" class="btn">{icon src=page_white_add}{$i18n->_('New')}</a>{/if}</div></h3></caption>
        <thead>
            <tr>
                <th>{$i18n->_('#')}</th>
                <th>{$i18n->_('Name')}</th>
        ...
        <tbody>
            {foreach $employees as $employee}
                <tr>
                    ...
                    <td>{if "employee/edit"|isAllowed}<a href="{$baseUrl}/employee/edit/id/{$employee->getIdEmployee()}" class="btn">{icon class=tip src=pencil title=$i18n->_('Edit')}</a>{/if}</td>
                    <td>
                    {if $employee->getStatusEmployee()==1}
                        {if "employee/delete"|isAllowed}<a href="{$baseUrl}/employee/delete/id/{$employee->getIdEmployee()}" class="btn deactivate">{icon class=tip src=delete title=$i18n->_('Deactivate')}</a>{/if}
                    {else}
                        {if "employee/reactivate"|isAllowed}<a href="{$baseUrl}/employee/reactivate/id/{$employee->getIdEmployee()}" class="btn">{icon class=tip src=tick title=$i18n->_('Reactivate')}</a>{/if}
                    {/if}
                    </td>
                    <td>{if "employee/tracking"|isAllowed}<a href="{$baseUrl}/employee/tracking/id/{$employee->getIdEmployee()}" class="btn">{icon class=tip src=book_open title=$i18n->_('Tracking')}</a>{/if}</td>
